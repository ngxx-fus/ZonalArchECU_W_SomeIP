...
void HeartCtl(void* arg) {
    SysEntry("HeartCtl");
    SysLog("HeartCtl: ZECU High-level control started. Target CCU: 10.0.0.100:%d", CCU_UDP_PORT);
    ...
    while (1) {
        ...
        if (ret == STAT_OKE) {
            SysLog("HeartCtl: Sent ECU State -> D0: %u, D1: %u, D2: %u, M0: %u, M1: %u, SyncNum: %u", 
            ...
            SysErr("HeartCtl: Failed to send ECU State! Error: %d", ret);
        }
