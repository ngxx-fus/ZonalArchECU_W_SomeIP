...
static void __IHF__RegisterGPIO(HBridge_t *bridge, uint8_t Index) {
	...
	ledc_timer_t SelectedTimer =
		(Index < 2) ? bridge->Config.Timer[0] : bridge->Config.Timer[1];

	gpio_reset_pin(bridge->IN[Index]);

	/* this is a comment - map channel to the selected motor timer */
	ledc_channel_config_t ledc_ch = {.gpio_num = bridge->IN[Index],
									 .speed_mode = bridge->Config.Mode,
...
ReturnCode_t HBridge_Apply(HBridge_t *Ptr) {
	...
	/* Control flow: update both timers and re-register all pins if pin flag is
	 * set */
	if (SafeFlagHas(&Ptr->Flag, eHBridge_ModifiedPin)) {
		SysLog(
			"HBridge_Apply(...): Pin configuration modified, applying changes");
	...
	/* Control flow: apply duty cycles based on motor speeds if speed flag is
	 * set */
	if (SafeFlagHas(&Ptr->Flag, eHBridge_ModifiedSpeed)) {
		...
		for (int32_t i = 0; i < 2; i++) {
			...
			ledc_set_duty(Ptr->Config.Mode, Ptr->Config.Channels[Base],
						  (Fwd > MaxDuty) ? MaxDuty : Fwd);
			ledc_update_duty(Ptr->Config.Mode, Ptr->Config.Channels[Base]);

			/* Control flow: set and update duty for reverse channel */
			ledc_set_duty(Ptr->Config.Mode, Ptr->Config.Channels[Base + 1],
						  (Rev > MaxDuty) ? MaxDuty : Rev);
